...
int main(int argc, char **argv)
{
    ...
    vector<int> shape_Y_train = {y_train->getShape()[0], output_softmax ? 2 : 1};
    vector<int> shape_Y_test  = {y_test->getShape()[0],  output_softmax ? 2 : 1};
    Tensor * Y_train = nullptr;
    Tensor * Y_test = nullptr;
    Tensor * z_train = Tensor::zeros(y_train->getShape());
    Tensor * z_test  = Tensor::zeros(y_test->getShape());

    if (output_softmax) {
        Y_train = Tensor::zeros(shape_Y_train);
        Y_test  = Tensor::zeros(shape_Y_test);
        ...
        Y_train = y_train->reshape(shape_Y_train);
